...
48px is the canonical authored source density for new player-class sprites and representative high-information prop redraws. Scout and Engineer render at 28px on the 32px battlefield grid. The Battle Tank has an approved 32px visual footprint, making it visibly heavier while remaining inside the same one-tile renderer cap. Source density no longer changes the physical map footprint.

This corrects the rejected first pass, which enlarged the 48px source to a 48px runtime sprite. The revised source adds tread rollers, moving treads, panel seams, engine vents, hatches, optics, rivets, tools, barrel highlights, wear, and layered armor. Scout and Engineer preserve two pixels of terrain clearance on each side; Battle uses the full tile without crossing it.
...
| --- | --- | --- |
| Pixel density comparison | `?visualQa=pixel_density_comparison` | `output/pixel-density-correction/battle-class-scale/runtime-comparison/` |
| Player Combat Matrix | `?visualQa=player_combat_matrix` | `output/pixel-density-correction/battle-class-scale/player-combat-matrix/` |
...
| Battle Tank boundary proof | `?devLevel=visual_density_slice&tankClass=battle` | `output/pixel-density-correction/battle-class-scale/battle-wall-adjacency/` |

All review routes were captured with the repository-required Playwright game client. Their `render_game_to_text` artifacts report integer Canvas2D coordinates, smoothing disabled for the QA boards, and no browser error file.
...
The sanctioned class-scale follow-up repeats the same case with the Battle Tank. Runtime text confirms both the selected and active class are `battle`; the inspected 32px sprite uses the complete open tile but does not paint over the adjacent brick.
...
- Battle Tank: wide heavy hull, large turret, permanent armor identity, and a sanctioned 32px visual footprint.
- 48px is source detail only; the battlefield composite is 28px for Scout/Engineer and 32px for Battle, and can never exceed one tile.
...
- Vehicle footprint regression: pass for 28px Scout/Engineer output, 32px Battle output, and the one-tile renderer maximum.
...
- Fine 48px source details necessarily collapse into fewer destination pixels at 28–32px, so class readability must continue to be judged at actual gameplay scale rather than from enlarged atlas previews.
