...
- Local session telemetry v1 is default-off and writes bounded lifecycle/result JSONL only when a path is explicitly configured. A second explicit private-playtest switch adds room keys, callsigns, chat, Colyseus-provided IPs, and internal identifiers; generated files stay under ignored `output/` and are not committed evidence. Real-SDK and multi-browser checks prove both pseudonymous and sensitive modes without logging per-frame inputs, positions, or snapshots.
