...
describe('CTF flag presentation model', () => {
  ...
  it('anchors the carried flag behind the exact moving tank position rather than its grid cell', () => {
    ...
    expect(movingRight).toMatchObject({ mirrorX: true, rotationQuarterTurns: 0 })
    expect(movingRight.x).toBeLessThan(103.25)
    expect(movingLeft).toMatchObject({ mirrorX: false, rotationQuarterTurns: 0 })
    expect(movingLeft.x).toBeGreaterThan(103.25)
    expect(movingUp).toMatchObject({ mirrorX: false, rotationQuarterTurns: 1 })
    expect(movingUp.y).toBeGreaterThan(220.5)
    expect(movingDown).toMatchObject({ mirrorX: false, rotationQuarterTurns: -1 })
