...
const ENCYCLOPEDIA_TOPICS: EncyclopediaTopic[] = [
  {
    ...
    label: 'Overview',
    helper: [
      'Tanchiki is a top-down tank campaign of classes, lanes, vision, and objectives.',
      'Boot Camp teaches live controls; Garage choices change tactics, not permanent stats.',
      'Win by defending, coordinating, capturing, outscoring, or breaking the target.',
    ],
    summary: [
      'Choose a class and Major Mod, learn in Boot Camp, then solve each battlefield objective.',
    ],
    entries: [
      { label: 'Boot Camp', description: 'Six replayable drills teach live mechanics.', visual: 'player-tank' },
      { label: 'Campaign', description: 'Clear missions, protect goals, and earn credits.', visual: 'campaign' },
      { label: 'Garage', description: 'Choose a tank class and one Major Mod.', visual: 'overdrive' },
      { label: 'Online', description: 'Shared sight and team color shape battles.', visual: 'online' },
  ...
  },
  {
    ...
    helper: [
      ...
      'Space fires; 1/2 use class kit; X activates the Major Mod; Hold E handles relay.',
      'P pauses. Back button, B, or Backspace navigates without leaving fullscreen.',
    ],
    summary: [
      'Keyboard and touch share one rule: tap to aim, hold to drive, then use the side controls.',
    ],
    entries: [
      { label: 'Move', description: 'Tap to pivot; hold to drive one tile.', visual: 'controls' },
      { label: 'Fire', description: 'Space or right Fire button shoots forward.', visual: 'player-tank' },
      { label: 'Class Kit', description: 'Use 1/2 or the right-side kit icons.', visual: 'bulwark' },
      { label: 'Major Mod', description: 'Use X or slide the right Mod control up.', visual: 'overdrive' },
      { label: 'Relay', description: 'Hold E or the left Relay control.', visual: 'portable-relay' },
      { label: 'Flag', description: 'R or Flag drops it for a handoff.', visual: 'ctf-flag' },
      { label: 'Pause', description: 'P opens Save And Quit or Restart.', visual: 'controls' },
      { label: 'Back', description: 'Back button, B, or Backspace backs out.', visual: 'controls' },
      { label: 'Fullscreen', description: 'F toggles; Escape only exits fullscreen.', visual: 'controls' },
    ],
  },
  {
    id: 'tanks',
    label: 'Tanks',
    helper: [
      'Scout, Engineer, and Battle Tank have fixed stats, shells, kits, and relay limits.',
      'Battle Tank trades speed for HE splash, Bulwark timing, and Traverse field control.',
      'Enemy roles remain readable: pressure, hunt, breach, or absorb punishment.',
    ],
    summary: [
      'Pick a doctrine, then use its real strengths instead of trying to erase its weaknesses.',
    ],
    entries: [
      { label: 'Scout', description: 'Fast recon; light AP; Decoy/Wire; 1 relay.', visual: 'scout-class' },
      { label: 'Engineer', description: 'Mine/Trap; 2 relays; slower reload.', visual: 'engineer-class' },
      { label: 'Battle Tank', description: 'Heavy HE splash; Bulwark + Traverse.', visual: 'battle-class' },
      { label: 'Basic', description: 'Pressures the base and fills lanes.', visual: 'basic-tank' },
      { label: 'Hunter', description: 'Fast enemy Scout forces movement.', visual: 'scout-tank' },
      { label: 'Breaker', description: 'Opens brick lanes and cover.', visual: 'breaker-tank' },
      { label: 'Armored', description: 'Five HP and a higher-value kill.', visual: 'armored-tank' },
    ],
  },
  {
    id: 'objectives',
    ...
    helper: [
      ...
      'CTF allows flag drops and handoffs; FFA rewards kills; Assault breaks the core.',
    ],
    summary: [
    ...
    entries: [
      ...
      { label: 'CTF', description: 'Carry home; R drops for recovery or handoff.', visual: 'ctf-flag' },
  ...
  },
  {
    ...
    entries: [
      ...
      { label: 'Bulwark', description: `1: ${BULWARK_DURATION_SECONDS}s field; absorbs ${BULWARK_CAPACITY}; recharges ${BULWARK_RECHARGE_SECONDS}s.`, visual: 'bulwark' },
      { label: 'Traverse', description: `2: strafe for ${TRAVERSE_DURATION_SECONDS}s without turning the gun.`, visual: 'traverse' },
      { label: 'Overdrive', description: '2x move; class duration: 5 / 4 / 2.5s.', visual: 'overdrive' },
      { label: 'Pontoon', description: 'One shared bridge across water.', visual: 'pontoon' },
      { label: 'Hedgehog', description: `One-use trap; destroyed by ${HEDGEHOG_REQUIRED_HITS} direct hits.`, visual: 'hedgehog' },
      { label: 'EMP', description: `${EMP_DISRUPT_SECONDS}s blackout / ${EMP_PULSE_PERIOD_SECONDS}s pulse / ${EMP_RADIUS_TILES}-tile radius.`, visual: 'emp' },
    ],
  },
  {
    id: 'terrain',
    ...
    helper: [
      ...
      'Biomes add mud, snow, reeds, gravel, ricochet plates, dust, and signal echoes.',
      'Radio towers link vision. Ammo stations recharge shells while you hold position.',
    ],
    summary: [
    ...
    entries: [
      ...
      { label: 'Water / Trees', description: 'Water blocks; vegetation conceals.', visual: 'water' },
      { label: 'Radio', description: 'Captured towers share team sight.', visual: 'radio' },
      { label: 'Ammo', description: 'Hold on a station to recharge shells.', visual: 'ammo' },
      { label: 'Swamp / Reeds', description: 'Wet ground slows; reeds conceal movement.', visual: 'swamp' },
      { label: 'Metal / Ricochet', description: 'Hard cover redirects routes.', visual: 'ricochet' },
      { label: 'Snow / Gravel', description: 'Biome material makes routes readable.', visual: 'snow' },
      { label: 'Dust / Echo', description: 'Dust and echoes complicate detection.', visual: 'echo' },
    ],
  },
  {
    id: 'back',
...
export class TanchikiGame {
  ...
  constructor(options: GameOptions = {}) {
    ...
    this.friendlyRemaining = this.normalizeFriendlyRemaining(
      this.savedRun?.friendlyRemaining,
      this.savedRun?.enemies,
    )
  ...
  startGame(levelId = this.currentLevelId) {
    ...
    this.friendlyRemaining = this.usesFiniteFriendlyRoster() ? this.getFriendlyRosterTotal() : 0
  ...
  private getFriendlyTargetCount() {
    ...
    const spawnCount = Math.max(
      this.currentObjective.friendlySpawns?.length ?? 0,
      this.currentLevel.friendlyLoadouts?.length ?? 0,
    )
    return Math.min(this.getActiveFriendlyLimit(), spawnCount)
  }
  ...
  private getActiveFriendlyLimit() {
    ...
    const declared = Math.max(
      this.currentObjective.activeFriendlyLimit ?? this.currentObjective.friendlyTotal ?? 0,
      this.currentLevel.friendlyLoadouts?.length ?? 0,
    )
  ...
  private usesFiniteFriendlyRoster() {
    return this.hasFriendlyRosterObjective()
      && typeof this.currentObjective.friendlyRosterTotal === 'number'
      && this.currentObjective.friendlyRosterTotal > 0
  }
  ...
  private normalizeFriendlyRemaining(value: unknown, savedEnemies: SavedTank[] = []) {
    if (!this.usesFiniteFriendlyRoster()) {
      return 0
    }
  ...
  private getObjectiveBriefingLine() {
    ...
    if (this.currentObjective.mode === 'team-battle') {
      return this.getFriendlyRosterTotal() > this.getActiveFriendlyLimit()
        ? `Objective: clear ${total} enemy tanks; allied reserve ${this.getFriendlyRosterTotal()}.`
        : 'Objective: fight with allies until enemy tickets are gone.'
    }
    ...
    return {
      ...
      hud: {
        ...
        enemies: this.currentObjective.mode === 'ffa' && this.hasContinuousEnemySpawns()
          ...
          : this.currentObjective.mode === 'team-battle'
            ? `Enemies active ${this.enemies.filter((tank) => tank.side === 'enemy').length}/${this.currentLevel.activeEnemyLimit} roster ${this.enemiesRemaining + this.enemies.filter((tank) => tank.side === 'enemy').length}/${this.currentLevel.enemyTotal}`
            : `Enemies remaining ${this.enemiesRemaining + this.enemies.filter((tank) => tank.side !== 'player').length}`,
        allies: this.usesFiniteFriendlyRoster()
          ? `Allies active ${this.getActiveFriendlyBotCount()}/${this.getActiveFriendlyLimit()} reserve ${this.friendlyRemaining}/${this.getFriendlyRosterTotal()}`
          : this.hasFriendlyRosterObjective()
            ? `Allies active ${this.getActiveFriendlyBotCount()}/${this.getActiveFriendlyLimit()}`
          : 'Allies not deployed',
  ...
  private getReadableObjectiveLine() {
    ...
    if (this.objectiveState.mode === 'team-battle') {
      const enemies = this.enemiesRemaining + this.enemies.filter((tank) => tank.side === 'enemy').length
      return this.usesFiniteFriendlyRoster()
        ? `Team battle: enemies ${enemies}/${this.currentLevel.enemyTotal}; allies ${this.getActiveFriendlyBotCount()}/${this.getActiveFriendlyLimit()} active, ${this.friendlyRemaining} reserve.`
        : `Team battle: enemies ${enemies}/${this.currentLevel.enemyTotal}; allies ${this.getActiveFriendlyBotCount()}/${this.getActiveFriendlyLimit()} active.`
    }
  ...
  private updateFriendlyRespawns(dt: number) {
    ...
    if (this.usesFiniteFriendlyRoster() && this.friendlyRemaining <= 0) {
      this.friendlyRespawnTimer = 0
      return
    }
  ...
  private destroyEnemy(enemy: Tank, bullet?: Bullet) {
    ...
    if (
      friendlyBot
      && (!this.usesFiniteFriendlyRoster() || this.friendlyRemaining > 0)
      && this.getActiveFriendlyBotCount() < this.getFriendlyTargetCount()
    ) {
