...
describe('online input tracker', () => {
  ...
  it('preserves buttons still held by another input source', () => {
    const input = new OnlineInputTracker()

    expect(input.setButton('up', true, 'keyboard')).toBe(true)
    expect(input.setButton('up', true, 'pointer')).toBe(true)
    expect(input.getCommand()).toMatchObject({ up: true })

    expect(input.setButton('up', false, 'keyboard')).toBe(true)
    ...
    expect(input.setButton('up', false, 'pointer')).toBe(true)
    expect(input.getCommand()).toMatchObject({ up: false })
  })
})
