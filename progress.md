...
- Focused release guard tests pass at 13/13, including malformed endpoints, revision drift, active rooms, expanded health payloads, expired rollback behavior, preview-artifact rejection, paginated run discovery, off-origin pagination rejection, valid production-root selection among multiple same-source runs, and a deployment-boundary backend drain recheck.
