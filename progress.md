...
- Echo Quarry follow-up: plain `?devLevel=echo_quarry` now opens the normal menu with Level 9 unlocked for class selection, while `?devLevel=echo_quarry&autostart=1` remains the gameplay smoke shortcut; moved one ammo station into the south staging area near player spawn and added a regression assertion for nearby ammo.

- Focused Echo Quarry follow-up validation passed: `npm.cmd run test -- src/game/game.test.ts src/game/levelReadability.test.ts` with 103 tests.

- Ammo visibility correction: moved the south staging ammo point off the fuel-barrel prop overlap to unobstructed tile `(16,26)`; focused Echo Quarry tests still pass with 103 tests.

- Final browser evidence refreshed with develop-web-game client: `output/web-game-echo-quarry-class-select-final` shows Tank Select reachable from plain Echo Quarry dev URL with Battle Tank selected; `output/web-game-echo-quarry-access-final` shows active Level 9 gameplay with visible ammo, active bots, finite friendly reserve, visible relay, and no console error files.
