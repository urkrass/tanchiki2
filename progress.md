...
- Focused lazy-runtime and input coverage passes at 2 files / 44 tests, including late-load disposal during page unload and suppression of Enter/Space leakage into the paused offline menu while the route boundary owns the screen. Production build now emits a 623,009-byte / 174,736-byte gzip entry and a 168,899-byte / 51,857-byte gzip online chunk. Initial JavaScript falls 161,816 bytes (20.62%) and 49,325 gzip bytes (22.01%); total JavaScript grows 7,083 bytes (0.90%) for the explicit boundary. The entry remains above the 500 kB warning and broad offline engine/renderer extraction is deliberately not disguised as complete.
...
- Closeout surface: PR #122. The initial exact-head Reviewer App run stopped only at the expected human architecture gate. Codex then found one primary-key leak during lazy loading; the repair and regression keep Enter/Space from reaching the paused offline menu while preserving Back recovery. Refresh both automated reviews on the repaired head, then stop for the human merge gate. Do not merge or deploy.
