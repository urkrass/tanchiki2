...
- Focused evidence: `npm.cmd test -- src/game/ai/botAi.test.ts src/game/game.test.ts` passed with 95 tests.
- Full validation evidence: `npm.cmd run validate` passed with 18 test files and 193 tests, production build, server smoke, harness validate/smoke, Reviewer App dry-run wrapper validation, and attended-v2 lifecycle telemetry smoke. Separate `npm.cmd run visual:contrast`, Product Review Warden, `npm.cmd run harness:deep-agent:stub-runtime`, `git diff --check`, and `git diff --cached --check` passed.
