...
try {
  ...
  await advance(40)
  state = await readState()
  assert(state.tankClasses.showcase.displayed === 'battle', 'Right did not preview Battle Tank')
  assert(state.progression.selectedTankClass === 'engineer', 'previewing Battle Tank changed the equipped class')
  await confirm()
