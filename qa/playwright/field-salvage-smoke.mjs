...
try {
  ...
  state = await readState(page)
  assert(state.player.salvage.shellProgress > 0.9, 'ammo progress did not approach a full track before payout')
  assert(state.player.shells === 8, 'ammo recovered before the progress track completed')
  await capture(page, '03-near-shell-payout.png', state)
  ...
  await capture(page, '04-shell-recovered.png', state)
  ...
  await capture(page, '05-burned-wreck.png', state)

  await advance(page, 8000)
  state = await readState(page)
  assert(state.wrecks.length === 0, 'burned wreck did not decay away')
  await capture(page, '06-decayed-route.png', state)
  ...
  await capture(page, '07-denial-fire.png', state)

  if (errors.length > 0) {
    throw new Error(`Browser errors:\n${errors.join('\n')}`)
  }
  fs.writeFileSync(path.join(outputDir, 'errors.json'), '[]\n')
  process.stdout.write(`${JSON.stringify({ ok: true, captures: 8, errors: 0 })}\n`)
} catch (error) {
