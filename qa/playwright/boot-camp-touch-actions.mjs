...
async function verifyCtfTouchDrop() {
  ...
  assert(state.tutorial.stepId === 'recover', `Touch transfer did not advance to recovery, received ${state.tutorial.stepId}`)
  ...
  await advance(page, 250)
  state = await readState(page)
  assert(state.objective.flag?.carrierId === 'player', 'Touch route did not recover the transferred flag')
  assert(state.tutorial.stepId === 'capture', `Touch recovery did not advance to capture, received ${state.tutorial.stepId}`)
